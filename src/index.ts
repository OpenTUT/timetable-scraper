...
function getTimetable(): Timetable | null {
	...
	const getSubject = (elm: HTMLElement): Subject | null => {
		...
		const term = getTextOrNull(
			elm.querySelector('span[id$="_lblTermName"]'),
		);
		const units = getTextOrNull(
			elm.querySelector('span[id$="_lblCredit"]'),
		);
		const staff = getTextOrNull(
			elm.querySelector('span[id$="_lblStaffName"]'),
		);
	...
	try {
		const belong = getTextOrNull(
			document.querySelector<HTMLSpanElement>(
				"#ctl00_bhHeader_lblBelong",
			),
		);

		const semester = document.querySelector<HTMLSelectElement>(
				"#ctl00_phContents_ddlTerm",
			)
				?.value === "1"
			? "spring"
			: "fall";
